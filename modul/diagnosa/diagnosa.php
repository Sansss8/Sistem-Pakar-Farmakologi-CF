...
switch ($_GET['act']) {
  ...
  default:
    if ($_POST['submit']) {
      ...
      $arbobot = array('0', '1', '0.8', '0.6', '0.4', '0.2', '0.0', '-0.6', '-0.8', '-1');
      $argejala = array();
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
	          <hr class = table-primary bg-primary> <table class='table table-bordered table-striped table-primary diagnosa'> 
          <th width=8%>No</th>
          <th width=10%>Kode</th>
      ...
      foreach ($argejala as $key => $value) {
        ...
        echo '<tr ><td>' . $ig . '</td>';
        echo '<td>B' . str_pad($r4['kode_gejala'], 3, '0', STR_PAD_LEFT) . '</td>';
      ...
      echo "<div class='callout callout-default'>Jenis obat yang dikonsumsi adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2) * 100 . " % (" . $vlpkt[1] . ")<br></h3>";
      ...
      echo $arspkt[$idpkt[1]];
      echo "</h4></div></div>
          <div class='box box-info box-solid'><div class='box-header with-border'><h3 class='box-title'>Kemungkinan lain:</h3></div><div class='box-body'><h4>";
      for ($ipl = 2; $ipl < count($idpkt); $ipl++) {
        echo " <h4><i class='fa fa-caret-square-o-right'></i> " . $nmpkt[$ipl] . "</b> / " . round($vlpkt[$ipl], 2) * 100 . " % (" . $vlpkt[$ipl] . ")<br></h4>";
      }
      echo "</div></div>
		  </div>";
    } else {
