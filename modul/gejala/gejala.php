...
?>
	<script type="text/javascript">
		function Blank_TextField_Validator() {
			if (text_form.nama_gejala.value == "") {
				alert("Nama Efek Samping Obat tidak boleh kosong !");
	...
	switch ($_GET['act']) {
		...
		default:
		  ...
		  <tr><td><input class='btn bg-olive margin' type=button name=tambah value='Tambah Efek Samping Obat' onclick=\"window.location.href='gejala/tambahgejala';\"><input type=text name='keyword' style='margin-left: 10px;' placeholder='Ketik dan tekan cari...' class='form-control' value='$_POST[keyword]' /> <input class='btn bg-olive margin' type=submit value='   Cari   ' name=Go></td> </tr>
          </table></form>";
			...
			if ($_POST['Go']) {
                ...
                Efek Samping Obat yang anda cari di temukan.
              </div>";
					$i = 1;
		  ...
		  <tbody>";
			 ...
			 <td align=center><a type='button' class='btn btn-success margin' href=gejala/editgejala/$r[kode_gejala]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	          <a type='button' class='btn btn-danger margin' href=\"JavaScript: confirmIt('Anda yakin akan menghapusnya ?','$aksi?module=gejala&act=hapus&id=$r[kode_gejala]','','','','u','n','Self','Self')\" onMouseOver=\"self.status=''; return true\" onMouseOut=\"self.status=''; return true\"><i class='fa fa-trash-o' aria-hidden='true'></i> Hapus</a>
                ...
                Maaf, Efek Samping Obat yang anda cari tidak ditemukan , silahkan inputkan dengan benar dan cari kembali.
              </div>";
				}
			} else {
		...
		case "tambahgejala":
          ...
          <br><br><table class='table table-bordered'>
		  <tr><td width=120>Nama Efek Samping Obat</td><td><input type=text autocomplete='off' placeholder='Masukkan efek samping obat baru...' class='form-control' name='nama_gejala' size=30></td></tr>
